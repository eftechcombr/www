#!/usr/bin/env python3
"""
Generate the featured image (1200x630px PNG) for the GLPI 12.0.0-rc1 blog post.

Design:
  - Dark navy gradient background (#0e1120 -> #1e2a4a) with a grid of dots
  - A terminal window showing `helm upgrade --install glpi eftech/glpi --version 2.12.0`
    and key cloud-native features (Valkey, MariaDB subchart, readOnlyRootFilesystem, HPA/PDB, S3 Backup)
  - Big title "GLPI 12.0.0-rc1", subtitle "Helm Chart 2.12.0 — Modernized Subcharts & Cloud-Native Hardening",
...
def draw_terminal_window(draw: ImageDraw, x: int, y: int, w: int, h: int) -> None:
    ...
    draw.text((x + w // 2, y + 21), "helm — eftech/glpi:2.12.0", fill=(170, 180, 210),
              font=font_title, anchor="mm")


def create_featured_image(output_path: str) -> None:
    """Create the 1200x630 OG image for the GLPI 12.0.0-rc1 post."""
    ...
    font_highlight = find_font(14, bold=True)
    ...
    draw.text((prompt_x + 20, line_y), " helm upgrade --install glpi eftech/glpi --version 2.12.0", fill=(240, 242, 250), font=font_cmd)
    line_y += 24

    terminal_lines = [
        ("Release \"glpi\" has been upgraded. Status: DEPLOYED (app: 12.0.0-rc1)", (120, 170, 255), False),
        ("✔ Subcharts: modern helmforge/valkey & helmforge/mariadb enabled", (82, 188, 74), True),
        ("✔ High Availability: PodDisruptionBudget & HPA configured (php-fpm / nginx)", (160, 190, 235), False),
        ("✔ Security: readOnlyRootFilesystem hardening active (empirically verified)", (160, 190, 235), False),
        ("✔ Storage: S3 files backup CronJob scheduled with custom credentials", (160, 190, 235), False),
        ("✔ Workload: priorityClassName support & Bitnami-style resourcesPreset", (160, 190, 235), False),
        ("✔ Operations: maintenance CronJob gated on database readiness", (140, 170, 215), False),
    ]
    ...
    line_y += 6
    draw.text((prompt_x, line_y), "$ docker pull ghcr.io/eftechcombr/glpi:12.0.0-rc1", fill=(225, 230, 245), font=font_cmd)

    # Title overlay below the terminal
    title_font = find_font(46, bold=True)
    draw.text((WIDTH // 2, ty + th + 66), "GLPI 12.0.0-rc1 & Helm Chart 2.12.0",
              fill=(255, 255, 255), font=title_font, anchor="mm")

    subtitle_font = find_font(23, bold=True)
    draw.text((WIDTH // 2, ty + th + 118), "Modernized Subcharts · Valkey Cache · Cloud-Native Hardening · S3 Backup",
              fill=(94, 170, 255), font=subtitle_font, anchor="mm")

    info_font = find_font(15)
    draw.text((WIDTH // 2, ty + th + 156), "Pre-release  ·  EF-TECH Containers & Helm  ·  Kubernetes-Ready ITSM",
